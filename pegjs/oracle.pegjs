{
  ...
  function createBinaryExpr(op, left, right) {
    ...
    if(isCompressionOperator(op)){
      if (right.join_type == 'outer_join')
      {
        ret.join_type = right.join_type;
        ret.outer_join_type = "left outer join";
        if(right.table){
          ret.outer_join_suggestion = `${left.table} LEFT OUTER JOIN ${right.table} ON ${left.table}.${left.column} ${op} ${right.table}.${right.column}` ;
        }
        else{
          ret.outer_join_suggestion = `${left.table}.${left.column} ${op} ${right.column}` ;
        }
  ...
  function isCompressionOperator(opStr) {
      return COMPRESSION_OPERATOR_MAP[opStr];
  }
  ...
  = __ cte:with_clause? __ KW_SELECT __
    ...
    l:limit_clause? {
      ...
      if(w) {
        let w_ = [];
        let f_ =[];
        //refine_outer_join(f_, w_, null, null, w);
      }
      return {
          with: cte,
          type: 'select',
...
column_ref
  ...
  / tbl:ident __ DOT __ col:column_without_kw __ OPERATOR_OUTER_JOIN {
...
column_name
  =  start:ident_start parts:column_part* pre:substitution_variable_prefix? parts2:column_part+ { return start + parts.join('') + pre + parts2.join(''); }
  /start:ident_start parts:column_part* { return start + parts.join(''); }

ident_name
  =  start:ident_start parts:ident_part* pre:substitution_variable_prefix? parts2:ident_part+ { return start + parts.join('') + pre + parts2.join(''); }
  /start:ident_start parts:ident_part* { return start + parts.join(''); }


substitution_variable
  = start:ident_start  parts1:ident_part* pre:substitution_variable_prefix parts2:ident_part* { 
